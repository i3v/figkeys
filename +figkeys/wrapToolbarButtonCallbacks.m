...
% described in "setOverwritable.m".
% 
...
htb1.ClickedCallback = ...
    @(varargin) figkeys_wrapper(hfig,'zoomin',mode);
...
htb2.ClickedCallback = ...
    @(varargin) figkeys_wrapper(hfig,'zoomout',mode);
...
htb3.ClickedCallback = ...
    @(varargin) figkeys_wrapper(hfig,'pan',mode);
...
htb4.ClickedCallback = ...
    @(varargin) figkeys_wrapper(hfig,'rotate3d',mode);
...
htb5.ClickedCallback = ...
    @(varargin) figkeys_wrapper(hfig,'datatip',mode);
...
htb6.ClickedCallback = ...
    @(varargin) figkeys_wrapper(hfig,'brush',mode);
...
function figkeys_wrapper(hfig,passedArg,mode)
